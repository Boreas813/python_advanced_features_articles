...
## 从Stream中读取数据
现在我们可以通过**XADD**向stream中添加数据了。向stream中添加数据看起来很清晰很明确，然而stream读取数据的方式看起来有些模糊。如果我们继续类比日志文件的话，比较清晰的表达方式是我们模仿Unix命令tail -f，这样我们可以开始监听新添加到stream中的信息。注意和Redis的list阻塞操作不同，list阻塞操作通过例如**BLPOP**这种阻塞*pop类型*操作让同一个元素只被一个客户端获取一次，在stream中全部的消费者都可以看到新添加进stream中的消息，就像很多个tail -f进程一起看同一个日志里写入了什么一样。用专业术语来说我们想让stream可以扇出(*fan out*)到多个客户端。
